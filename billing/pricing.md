---
icon: credit-card
description: Тарифы, токены, способ оплаты — за что списываются деньги и где смотреть расход.
---

# Тарификация и баланс

![Страница «Тариф и использование»](../.gitbook/assets/shot-billing.png)

## Тарифы

Тариф — основной способ платить за нейросеть: он даёт пакет токенов каждый месяц. Раздел **Тариф и использование** показывает текущий тариф и сравнение всех планов: Бесплатный, Старт, Авито, Профи, Команда, а также Компания — с условиями «индивидуально / по договорённости». Оплата помесячно или на год со скидкой −20%.

{% hint style="info" %}
**Индивидуальная цена.** Крупным клиентам поддержка может выставить персональную цену на тариф — тогда в кабинете отображается она, а не общая сетка.
{% endhint %}

## Токены

Деньги внутри кабинета — это токены, двух видов (на Панели управления: «тарифные · докупленные»):

* **Тарифные** приходят пакетом с тарифом каждый месяц.
* **Докупленные** — пополнение в разделе **Баланс** в любой момент.

| Операция | Как считается |
| --- | --- |
| Ответ ИИ в чате | По объёму ответа. Чем короче, тем дешевле. Есть потолок за один ответ. |
| Ответ агента на отзыв | По обычному курсу ответа агента |
| Ручные сообщения оператора | Бесплатно |
| Генерация фото | Фиксированная цена за картинку (зависит от модели) |
| Генерация текста объявления | По объёму |

{% hint style="warning" %}
**Токены кончились — агенты останавливаются.** Бот молча перестаёт отвечать, диалоги копятся в «Ждут ответа». Пополните баланс или дождитесь продления тарифа — уведомление о низком остатке приходит заранее.
{% endhint %}

## Оплата и чеки

Оплата картой через ЮKassa. Чек приходит на почту — поэтому почта аккаунта обязательна.

Текущий баланс и история списаний — в разделе **Баланс**, там же разбивка трат по типам операций и порог «низкого баланса» для оповещений.

![История списаний](../.gitbook/assets/screen-billing.png)

## Способ оплаты и автопродление

Блок **«Способ оплаты»** в разделе «Тариф и использование» показывает сохранённую карту и управляет автопродлением.

**Как сохранить карту.** При оплате тарифа включите «Продлевать автоматически» — карта сохранится в ЮKassa, и тариф будет продлеваться сам в день окончания. Дата и сумма видны заранее строкой «Следующее списание».

**Выключить автопродление.** Переведите тумблер **«Автопродление тарифа»** в выключенное состояние — тариф не продлится сам, а просто доработает до оплаченной даты.

**Отвязать карту.** Кнопка **«Отвязать карту»** стирает способ оплаты и выключает автопродление — и тарифа, и дополнительных услуг с этой карты. Оплаченный период не сгорает: тариф доработает до конца. При следующей оплате карту можно сохранить снова.

{% hint style="warning" %}
**Если автосписание не прошло** (например, на карте не хватило средств), повторных попыток списания не будет: придёт уведомление в кабинете и письмо на почту. Продлите тариф вручную в разделе «Тариф и использование» или проверьте карту.
{% endhint %}

## Дополнительные услуги

В разделе «Тариф и использование» блок **«Дополнительные услуги»** позволяет докупить к тарифу: дополнительный аккаунт Авито, дополнительного AI-агента или дополнительного менеджера.

Каждая дополнительная услуга — самостоятельная подписка: у неё **свой срок** (месяц или год со скидкой −20%) и **свой тумблер автопродления**, независимый от тарифа. Кнопка «Отменить» прекращает продление конкретной услуги — оплаченный срок она доработает.
